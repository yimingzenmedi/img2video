...
opt = Config('training_center.yml')
...
if __name__ == '__main__':
    ...
    model_dir = os.path.join(opt.TRAINING.SAVE_DIR, session, "centerEsti")
    ...
    if opt.TRAINING.RESUME:
        path_chk_rest = utils.get_last_path(model_dir, f'centerEsti_model_latest.pth')
    ...
    for epoch in range(start_epoch, opt.OPTIM.NUM_EPOCHS + 1):
        ...
        if epoch % opt.TRAINING.VAL_AFTER_EVERY == 0:
            ...
            if psnr_val_rgb > best_psnr:
                ...
                torch.save({'epoch': epoch,
                            ...
                            }, os.path.join(model_dir, f"centerEsti_model_best.pth"))
            ...
            torch.save({'epoch': epoch,
                        'state_dict': model_restoration.state_dict(),
                        'optimizer': optimizer.state_dict()
                        }, os.path.join(model_dir, f"centerEsti_model_epoch_{epoch}.pth"))
        ...
        torch.save({'epoch': epoch,
                    'state_dict': model_restoration.state_dict(),
                    'optimizer': optimizer.state_dict()
                    }, os.path.join(model_dir, f"centerEsti_model_latest.pth"))
